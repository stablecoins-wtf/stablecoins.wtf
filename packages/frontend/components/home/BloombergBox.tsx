...
export const BloombergBox: FC<BloombergBoxProps> = ({children, hideTopBar, isClosable, onClosed, title, ...props}) => {
  return <>
        ...
        {!hideTopBar && (
          <div tw="sticky -top-2 z-50 bg-black bg-opacity-50 backdrop-blur flex justify-between text-bbg-gray2 text-sm p-2 px-3 -mt-2 -mx-3">
            ...
            <div tw="flex-shrink-0 flex items-center select-none space-x-4">
              {isClosable
                ? <button tw="flex items-center space-x-2 self-center text-white bg-white bg-opacity-0 px-1 -mr-1 hover:bg-opacity-10"
                  onClick={() => onClosed?.()}>
                  <Image src={topbarCrossIcon} width={10} height={10} />
                  <span>Hide</span>
                </button>
                : <>
